...
const stop = () => {
  if (timerRunning) {
    ...
    timeRecord!.innerHTML = `stopped time: ${timeFormatter(
      stopTime - startTime)}`;
  } else {
...
const elapse = () => {
  if (timerRunning) {
    ...
    timeRecord!.innerHTML = `elapsed time: ${timeFormatter(
      elapsedTime)}`;
  } else {
    message!.innerHTML = 'start timer first';
  }
